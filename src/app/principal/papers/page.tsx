...
/** §5.9 Question Papers. A test's papers are managed per subject — each
 * subject's file is uploaded, mapped and turned into an answer card
 * independently. Upload/mapping/answer-card generation are all simulated
 * with local state and timed transitions; nothing is actually parsed. */
export default function PapersPage() {
  ...
  function createTest() {
    ...
    setToast(`${name} created with ${createDraft.subjects.length} subject${createDraft.subjects.length === 1 ? "" : "s"}. Upload each subject's paper below.`);
  }
