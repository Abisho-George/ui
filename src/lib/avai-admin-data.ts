...
export const statusMix = accountStatuses
  .map((status) => ({ status, count: adminSchools.filter((s) => s.status === status).length }))
  .filter((r) => r.count > 0);
...
export const pipelineBoard: PipelineColumn[] = pipelineStages.map((stage) => {
